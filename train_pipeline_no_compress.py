...
class VAEAtomCompressor(nn.Module):
    ...
    Output: z [latent_dim], mu, logvar, attn_weights [1, N]
    """
...
class AtomCountDecoder(nn.Module):
    """MLP head: latent z -> positive atom count (scalar)."""

    def __init__(self, latent_dim: int = LATENT_DIM, hidden_dim: int = HIDDEN_DIM) -> None:
        super().__init__()
...
class LatticeDecoder(nn.Module):
    """MLP head: latent z -> normalized lattice L_norm in R^6 (parallel to AtomCountDecoder)."""
...
def kl_divergence(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    """Standard Gaussian KL, summed over latent dims."""
...
def compute_multitask_loss(
    ...
    *,
    ...
    Combined loss:
      w_vae     * KL(mu, logvar)
      w_atom    * (MSE + PoissonNLL) on atom count
      w_lattice * MSE on L_norm (CDVAE lattice_loss)
    """
    if mse_fn is None:
        mse_fn = nn.MSELoss()
    ...
    atom_mse = mse_fn(rate, target)
    atom_poisson = poisson_fn(rate, target)
    atom_total = atom_mse + atom_poisson
    lattice_mse = lattice_loss(pred_L, true_L, mse_fn=mse_fn)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Train VAE-Attention-Atom-Predictor (no latent compression)")
...
def train_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    print("=== VAE-Attention-Atom-Predictor pipeline (no compression, latent=128) ===")
